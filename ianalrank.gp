\p 250
F(k)=J(k-1,X)*sinv(k,X)^(dv+1)
N=1000; mx=dv; L; X; SP=mx+2; default(seriesprecision,SP+1); H=vector(SP);
...
{QDc(x)=local(A); print(precision(IEEE(x),18)); print(0); print(0); print(0);}
...
MM=100; while(log(10^(-17)+abs(ev(P,MM,N)))<-37,MM/=1.1); MM*=1.1;
write1("datafiles/param_data",STR,",",precision(IEEE(MM),18));
print("Now working backwards..."); \\ ends up in /dev/null
l=-10; while(log(10^(-17)+abs(ch(2^l,20)))<-37,l+=1); l-=1; s=l;
...
while(2^l*31/32<MM,\
      while(log(10^(-17)+abs(ch(2*2^l,n)))>-37,n+=5); setup(l,n);l+=1);
default(log,0); write("datafiles/param_data",",",l-s);
coeffs(j)=for(i=0,19,print("PS ",i); QDc(polcoeff(polcoeff(P,j,L),i,X)));

STRs=concat(dSTR,"S.txt"); default(logfile,STRs); default(log,1); coeffs(0);
for(d=1,dv,print("TACKS ",d-1); QDc(polcoeff(polcoeff(P,d,L),0,X)));
default(log,0);
